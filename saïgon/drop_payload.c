...
void spawn_bundle_binary_with_priv_port(mach_port_t privileged_task_port,
  ...
  target_pid = spawn_bundle_binary(privileged_task_port, spawn_context_task_port, binary, argv, envp);
    ...
    // Check if out target_pid is set
//    if(target_pid == -1) {
        printf("[Error]: Process was not launched.\n");
//        return;
...
//    }
  ...
  if (target_task_port == MACH_PORT_NULL) {
      printf("[Error]: Can't find pid of the new process %s..\n", argv[0]);
//      return;
  }
...
void reload_launchd(mach_port_t privileged_task_port, mach_port_t spawn_context_task_port) {
    ...
    printf("[BEGIN]: reloading..\n");
    
    // exec the custom debugserver binary
    char* argv[] = {"reload", NULL};
    
    spawn_bundle_binary_with_priv_port(privileged_task_port, spawn_context_task_port, "reload", argv, NULL);
    
    printf("[END]: Finished running 'reload'\n");
}
...
//    printf("[*] Running ziva..\n");
    char* argv[] = {"ziva", NULL};
    char* envp[] = {NULL};
    spawn_bundle_binary_with_priv_port(privileged_task_port, spawn_context_task_port, "ziva", argv, envp);
